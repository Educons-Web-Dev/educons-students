...
export const PromoteUserModal = () => {
  ...
  const { mutate: promoteUserMutation } = trpc.useMutation('user.promote', {
    onSuccess: () => {
      notify('Korisnik je postao admin!', 'success');
      handleClose();
  ...
  const handlePromoteUser = () => {
    if (selectedUser?.email) promoteUserMutation({ email: selectedUser.email });
  };

  return (
    <div className="bg-white text-black p-10 rounded-md shadow-md">
      <h2 className="text-lg lg:text-xl mb-[2px]">
        Da li ste sigurni da želite da korisniku <span className="font-bold">{selectedUser?.name}</span> date admin
        privilegije?
      </h2>
